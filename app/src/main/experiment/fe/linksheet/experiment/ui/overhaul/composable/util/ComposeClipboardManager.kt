...
const val PLAIN_TEXT_LABEL = "plain text"

val LocalComposeClipboardManager = staticCompositionLocalOf<ComposeClipboardManager> {
    error("LocalComposeClipboardManager")
}

typealias AndroidClipboard = android.content.ClipboardManager

class ComposeClipboardManager(val clipboardManager: AndroidClipboard) : ClipboardManager {

    constructor(context: Context) : this(context.getSystemService<AndroidClipboard>()!!)

    override fun setText(annotatedString: AnnotatedString) {
        setText(annotatedString.convertToCharSequence())
    }

    fun setText(charSequence: CharSequence) {
        clipboardManager.setPrimaryClip(ClipData.newPlainText(PLAIN_TEXT_LABEL, charSequence))
    }
    ...
    override fun getClip(): androidx.compose.ui.platform.ClipEntry? {
    ...
    override val nativeClipboard: AndroidClipboard
