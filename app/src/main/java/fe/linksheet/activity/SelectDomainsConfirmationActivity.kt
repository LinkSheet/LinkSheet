...
class SelectDomainsConfirmationActivity : ComponentActivity() {
    companion object {
        ...
        fun start(context: Context, callingPackage: String, callingComponent: String, domains: StringParceledListSlice) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent?.action == ACTION_CONFIRM) {
            ...
            val domains = IntentCompat.getParcelableExtra(intent, EXTRA_DOMAINS, StringParceledListSlice::class.java)?.list
                        ...
                        Box(modifier = Modifier.fillMaxWidth()) {
                            val dialog = dialogHelper(
                                ...
                                DialogColumn {
                                    ...
                                    SubtitleText(subtitle = stringResource(id = R.string.domain_selection_confirmation_subtitle, appLabel))
                                    DialogSpacer()
                                    DialogContent(
                                        items = state,
                                        key = { it },
