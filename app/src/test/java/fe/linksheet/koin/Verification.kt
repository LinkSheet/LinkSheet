...
class Verification(val module: Module, extraTypes: List<KClass<*>>, injections: List<ParameterTypeInjection>? = null) {
    ...
    private val parameterInjectionIndex: Map<String, List<String>> = injections?.associate { inj ->
        inj.targetType.getFullName() to inj.injectedTypes.map { it.getFullName() }.toList()
    }.orEmpty()
    ...
    fun verify(): List<String> {
        ...
        factories.forEach { factory ->
            if (factory !in verifiedFactories.keys) {
                val injectedDependencies = verifyFactory(missingDefinitions, factory, definitionIndex)
    ...
    private fun verifyFactory(
        missingDefinitions: MutableList<String>,
        ...
        verificationByStatus[VerificationStatus.MISSING]?.let { list ->
            val definitionStr = list.joinToString(separator = ", ") { "'$it'" }
            val definition = generateInjectionDefinition(beanDefinition, list)

            val code = generateInjectionCodeComplex(beanDefinition, definition)

            val classNames = list.map { it.type.qualifiedName }
            val classNamesStr = classNames.joinToString(", ") { "'$it'" }
            val singleDefsStr = classNames.joinToString("\n") { "  * singleOf(::$it)" }
            val errorMessage = """
            |* ----- > Missing definition for '$definitionStr' in definition '$beanDefinition'.
            |- Fix your Koin configuration
            |1- add missing definition for type(s) $classNamesStr:
            |$singleDefsStr
            |${code}
            """.trimMargin()
...
//            throw MissingKoinDefinitionException(errorMessage)
    ...
    private fun generateFixDefinition(first: VerifiedParameter): String {
        ...
        return """add missing definition for type '$className'. like: singleOf(::$className)"""
    }
    ...
    private fun generateInjectionCodeComplex(
    ...
    private fun generateInjectionDefinition(
        beanDefinition: BeanDefinition<*>,
        parameters: List<VerifiedParameter>
    ): String {
        val parameters = parameters.joinToString(separator = ", ") { p ->
            "${p.type.qualifiedName}::class"
        }

        val className = beanDefinition.primaryType.qualifiedName
        return """definition<$className>($parameters)"""
    }
    ...
    private fun verifyConstructor(
        ...
        return constructorParameters.map { constructorParameter ->
            ...
            val alreadyBoundFactory =
                verifiedFactories.keys.firstOrNull { ctorParamClass in listOf(it.beanDefinition.primaryType) + it.beanDefinition.secondaryTypes }
