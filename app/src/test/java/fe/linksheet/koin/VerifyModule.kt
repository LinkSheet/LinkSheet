 ...
 * @throws org.koin.test.verify.MissingKoinDefinitionException
 */
@OptIn(KoinExperimentalAPI::class)
fun Module.verify(extraTypes: List<KClass<*>> = emptyList(), injections: List<ParameterTypeInjection>? = emptyList()): List<String> {
    return Verify.verify(this, extraTypes, injections)
}
...
object Verify {
     ...
     */
    @OptIn(KoinExperimentalAPI::class)
    fun verify(module: Module, extraTypes: List<KClass<*>> = emptyList(), injections: List<ParameterTypeInjection>? = null): List<String> {
        val verification = Verification(module, extraTypes, injections)
        println("Verifying module '$module' ...")

        val (missingDefinitions, time) = measureTimedValue {
            verification.verify()
        }
